...
sub date
{
    ...
    return $self->h->span({ class => 'partial-date' }, [
        join('-',
             $self->text($field->field('year'),  { maxlength => 4, placeholder => l('YYYY'), size => 4, class => 'form-control' }),
             $self->text($field->field('month'), { maxlength => 2, placeholder => l('MM'), size => 2, class => 'form-control' }),
             $self->text($field->field('day'),   { maxlength => 2, placeholder => l('DD'), size => 2, class => 'form-control' }))
    ]);
}
